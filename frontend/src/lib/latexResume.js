...
function parseHeadingFromLatex(latex = "") {
  ...
  // Backward compatibility: older generated content escaped this separator as \$|\$.
  const normalizedContactString = contactString.replace(/\\\$\|\\\$/g, "$|$");
  const contactParts = normalizedContactString.split(/\$\|\$/).map((part) => part.trim()).filter(Boolean);
  const plainParts = [];
...
export function formToLatex(formState = defaultResumeState) {
  ...
  const phoneContact = escapeLatex(heading.phone?.trim() || "Phone");
  const locationContact = escapeLatex(heading.location?.trim() || "Location");
...
\begin{center}
  ...
  \small ${phoneContact} $|$ ${locationContact} $|$ \href{mailto:${escapeLatex(heading.email || "your@email.com")}}{\underline{${escapeLatex(heading.email || "your@email.com")}}} $|$
